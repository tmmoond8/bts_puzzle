...
const StyledGameClear = styled.div`
  width: 100%;
  height: 100%;
...
const GameClear = (props: IProps) => {
  const { gameReady, time, nextGame } = props;
  return (
    <StyledGameClear>
      <div>
        <h3>Clear !!!</h3>
        <h3>Your Record : {(time / 1000).toFixed(2)}</h3>
        {nextGame && <h2 onClick={() => (Router.push(nextGame), gameReady())}>
          Next Game
        </h2>}
